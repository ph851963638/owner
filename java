...
优点： 1.提升开发效率  降低编程人员负担
       ...
       继承：  意义： 父类（基类 超类）和子类 
         ...
         特点：只有抽象类才可以放置抽象方法
               ...
               Map;
               集合：  集合能够解决的问题：
               集合可以理解为是一种更高级的数组，可以保存多条数据。
               本质： java 的官方开发人员基于Java 的一些基础内容（数组）等等，创建了一些接口和类
               然后使用这些接口和类的对象来更加灵活的保存或处理多条数据。
               Object 是一个比较特殊的类，在java中有这样的一条设定，Object类是任何类的父类，是一种万能的类型；
               ...
               LinkedList
               set  
               无序集合   放入元素时会有特定的算法为元素排序， 跟先后顺序也五官，因此没有跟List一样的下表去表示元素，不可重复
              Map  功能性强大，set和List    
              键（Key）   值（Value）
              
              总结： 集合框架（深，难，复杂）
              零基础（List Set、Map）各自特点；
              可以灵活使用List 
              初步理解API的意义，学会查看API文档来调用我们希望调用的方法
              有基础：分别分析和研究集合的源码
              重点模拟和实现一种集合，比如List ArrayList(); (面试常考)
